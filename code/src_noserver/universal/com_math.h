...
static float I_fmax(float x, float y)
{
	if ((float)(x - y) < 0.0)
	{
		return y;
...
static float I_fmin(float x, float y)
{
	if ((float)(y - x) < 0.0)
	{
		return y;
...
static float I_fclamp(float val, float min, float max)
{
	return I_fmax(min, I_fmin(val, max));
}

void TRACK_com_math(void);
long double I_normCDF(long double);
float random(void);
float crandom(void);
